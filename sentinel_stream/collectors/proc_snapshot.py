...
def _win_process_chain() -> List[str]:
    ...
    chains = []
    try:
        cmd = ["powershell", "-NoProfile", "-Command", "Get-CimInstance Win32_Process | Select-Object Name,ProcessId,ParentProcessId | ConvertTo-Json"]
...
def process_snapshot(host: str) -> Iterable[Event]:
    ...
    # POSIX: ps
    try:
        out = subprocess.check_output(["ps", "-eo", "ppid=,pid=,comm="], text=True, encoding="utf-8", errors="replace")
        sample = "\n".join(out.splitlines()[:200])
        yield Event(host=host, source="proc_snapshot", type="proc.snapshot", data={"chain": sample})
    except Exception:
        yield Event(host=host, source="proc_snapshot", type="proc.snapshot", data={"chain": "ps_unavailable"})
